...
| F-014 | Testing | Bezel swipe to launch task home | -- | Multi-finger (2+) swipe up from bottom canvas edge (bezel-in) opens task home. Gesture research confirms no system conflicts. Detection: DOWN in bottom 1% of page height + 2+ pointers + >150px upward + <600ms. Page height fetched dynamically (no hardcoded default). Configurable target (tab, project) planned. Currently testing on-device. |
