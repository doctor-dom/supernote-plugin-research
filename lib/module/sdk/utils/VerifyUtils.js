...
import { Element, ElementDataAccessor } from "../../model/Element.js";
...
export const verifyElement = element => {
   ...
   */
  const validTypes = [Element.TYPE_STROKE, Element.TYPE_TITLE, Element.TYPE_PICTURE, Element.TYPE_TEXT, Element.TYPE_TEXT_DIGEST_QUOTE, Element.TYPE_TEXT_DIGEST_CREATE, Element.TYPE_LINK, Element.TYPE_GEO, Element.TYPE_FIVE_STAR];
  ...
  if (elementType === Element.TYPE_TEXT || elementType === Element.TYPE_TEXT_DIGEST_QUOTE || elementType === Element.TYPE_TEXT_DIGEST_CREATE || elementType === Element.TYPE_LINK || elementType === Element.TYPE_TITLE) {
 ...
 */
function isElementDataAccessor(obj) {
  if (!obj || typeof obj !== 'object') return false;
  if (obj instanceof ElementDataAccessor) return true;
  const hasSize = typeof obj.size === 'function';
  const hasGet = typeof obj.get === 'function';
  const hasAsyncIterator = typeof obj[Symbol.asyncIterator] === 'function';
 ...
 */
function verifyElementAccessors(element) {
  if (element.angles !== undefined && element.angles !== null) {
    if (!isElementDataAccessor(element.angles)) {
      throw new APIError(107, 'angles must be an ElementDataAccessor object');
    }
  }
  if (element.contoursSrc !== undefined && element.contoursSrc !== null) {
    if (!isElementDataAccessor(element.contoursSrc)) {
 ...
 */
export function verifyParams(schema, params, options) {
      ...
      case 'object':
        {
          ...
          if (rule.properties) {
            verifyParams(rule.properties, value, {
              allowUnknown,
              rootName: path
            });
          }
          ...
          if (rule.items) {
            value.forEach((item, idx) => {
              ...
              if (itemRule.type === 'object' && itemRule.properties) {
                verifyParams(itemRule.properties, item, {
                  allowUnknown,
                  rootName: itemPath
                });
      ...
      case Element.TYPE_STROKE:
        {
          ...
          verifyParams({
            ...
            points: {
              type: 'object',
              custom: (v, p) => {
                if (v != null && !isElementDataAccessor(v)) throw new APIError(107, `${p} must be an ElementDataAccessor object`);
              }
            },
            pressures: {
              type: 'object',
            ...
            eraseLineTrailNums: {
              type: 'object',
              custom: (v, p) => {
                if (v != null && !isElementDataAccessor(v)) throw new APIError(2107016, `${p} must be an ElementDataAccessor object`);
              }
            },
            flagDraw: {
              type: 'object',
            ...
            markPenDirection: {
              type: 'object',
              custom: (v, p) => {
                if (v != null && !isElementDataAccessor(v)) throw new APIError(1072018, `${p} must be an ElementDataAccessor object`);
              }
            },
            recognPoints: {
              type: 'object',
      ...
      case Element.TYPE_GEO:
        {
          ...
          verifyParams(GeometrySchema, root.geometry, {
            allowUnknown: true,
            rootName: `${rootName}.geometry`
          });
          break;
        }
 ...
 */
export const ElementStrokeSchema = {
  type: 'object',
  properties: {
    uuid: {
      type: 'string',
